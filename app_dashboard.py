...
if admin_url:
    ...
    # Now connect to the todos database with admin to set schema ownership
    parsed = urlparse(admin_url)
    admin_todos_url = urlunparse((
        parsed.scheme,
        parsed.netloc,
        "/todos",
        parsed.params,
    ...
    conn = psycopg2.connect(admin_todos_url)
    conn.autocommit = True
    cursor = conn.cursor()
    ...
    # Use TODOS_DATABASE_URL if provided, otherwise construct it
    if not todos_database_url:
        todos_database_url = urlunparse((
            parsed.scheme,
            f"todos:{todos_password}@{parsed.netloc.split('@')[-1]}",  # Extract just host:port
            "/todos",
            parsed.params,
            "sslmode=require",
            parsed.fragment,
